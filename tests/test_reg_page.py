...
from settings import url_base_page, invalid_name, valid_email_or_phone, valid_password, valid_first_name, \
    valid_last_name, valid_region, invalid_email_or_phone, invalid_password, valid_password2, random_int, first_name_en, \
...
class TestRegPage():
    ...
    @pytest.mark.parametrize('region', valid_region)
    @pytest.mark.xfail
    def test_RT025_registration_with_valid_data(self, browser, first_name, last_name, email_phone, password, region):
        """ Поле при вводе названия региона выдает выпадающий список, который исчезает, при нажатии на экран в любом месте,
        а так же исчезает соответствующие строки html-кода, в результате автоматически выбрать город из выпадающего списка невозможно.
        Тест необходимо провести вручную (RTC-03)"""
        reg_page = RegPage(browser, url_base_page)
        reg_page.open_reg_page()
        reg_page.registration_with_valid_data(first_name, last_name, email_phone, password, region)
